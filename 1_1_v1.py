...
# ---------------------- Model (same-spatial-size CNN) ----------------------
class SameSizeCNN(nn.Module):
    """A simple CNN that preserves spatial dimensions (no pooling).
    This avoids shape mismatches when input H,W are not divisible by powers of 2.
    """
...
if __name__ == "__main__":
    ...
    # ---------------- model ----------------
    model = SameSizeCNN(in_channels=input_len).to(device)
    ...
    im0 = axes[0].imshow(
        actual, cmap="gray_r", vmin=tmin, vmax=tmax, interpolation="nearest"
    )
