...
base_dir = Path(script_dir).resolve().parents[3] / "Dataset"
...
num_epochs = 3  # CHANGED: was 5
train_window_stride = 4  # CHANGED: keep every 4th training window
lr = 1e-4
...
# CHANGED: thin training windows for speed (keep every 4th window)
train_tuples = train_tuples[::train_window_stride]
...
plt.ylabel("SSIM")
plt.title("Figure 1: Time series of SSIM for test data (2024) of SwinLSTM")
plt.grid(alpha=0.3)
...
plt.title("Figure 2: Time series of MAE for test data (2024) of SwinLSTM")
plt.grid(alpha=0.3)
plt.xticks(rotation=25)
plt.tight_layout()
...
plt.title("Figure 3: Time series of RMSE for test data (2024) of SwinLSTM")
plt.grid(alpha=0.3)
plt.xticks(rotation=25)
plt.tight_layout()
