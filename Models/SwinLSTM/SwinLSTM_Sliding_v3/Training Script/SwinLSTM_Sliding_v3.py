...
# ---------------- CONFIG ----------------
# NEW: top-level folder containing subfolders 2020,2021,2022,2023,2024
folder_path = "../../Dataset"
out_dir = "SwinLSTM_Sliding_v3"
...
batch_size = 6
lr = 4e-5  # slightly lower LR for more stable training with larger model
epochs = 80
val_split = 0.20
seed = 42
...
# SwinLSTM-specific (increased window + heads)
WINDOW_SIZE = 8  # spatial window size for local self-attention
ATTN_HEADS = 8

# Loss weights
SSIM_WEIGHT = 0.12  # weak SSIM term (if scikit-image available)
GRAD_WEIGHT = 0.8  # gradient loss weight (on absolute fields)

# DataLoader workers
# On Windows use 0 to avoid multiprocessing spawn issues (safe & simple)
NUM_WORKERS_TRAIN = 0
NUM_WORKERS_VAL = 0
...
# ---------------- Dataset (collecting by year folders, reserve 2024 as test) ----------------
class SlidingMaskDataset(TorchDataset):
    def __init__(
        ...
        # list year directories and filter by include/exclude
        ...
        # apply include/exclude lists if provided
...
# ---------------- SwinLSTM cell & model + refinement head ----------------
...
class ResidualSwinLSTMWithRefine(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + self.shortcut(last)  # residual shortcut to help local correction
        return out


# ---------------- Helper losses and metrics (unchanged logic) ----------------
...
# chronological split first 80% -> train, last 20% -> val (no shuffling)
...
# Use shuffle=False to keep strictly chronological batching as you requested
...
# ---------------- climatology computed from training targets (training years only) ----------------
...
# Cosine scheduler for smooth LR decay
sched = torch.optim.lr_scheduler.CosineAnnealingLR(
    opt, T_max=max(1, epochs), eta_min=1e-7
)
...
for epoch in range(1, epochs + 1):
    ...
    for X, y in pbar:
        ...
        with autocast():
            ...
            # optional SSIM term (computed on CPU via scikit-image)
    ...
    if epoch % PRINT_EVERY == 0:
        ...
        print(
            f"Epoch {epoch:03d} Train={train_loss:.6f} Val={val_loss:.6f} | VAL MSE={mse:.6f} MAE={mae:.6f} RMSE={rmse:.6f} SSIM={mean_ssim_val} | lr={current_lr:.2e}"
        )
    ...
    if epoch % 10 == 0:
        torch.save(
            model.state_dict(), os.path.join(out_dir, f"checkpoint_epoch{epoch}.pth")
        )
...
# ---------------- inference on validation set for bias correction & blending (unchanged) ----------------
...
# ---------------- Build the requested sample in the DOWNSAMPLED timeline (unchanged) ----------
...
# ----------------- lat/lon and plotting (unchanged) ---------------
# ----------- FIX: Find .nc file inside year subfolders -----------
def find_first_nc_recursive(root_folder):
    ...
    raise FileNotFoundError(
        "❌ No .nc file found anywhere under Dataset/. Check Dataset/20xx folders."
    )


# Use recursive search so it works with Dataset/2020/*.nc, Dataset/2021/*.nc, ...
nc0 = find_first_nc_recursive(folder_path)
print("Found reference .nc file:", nc0)
# ---------------------------------------------------------------
