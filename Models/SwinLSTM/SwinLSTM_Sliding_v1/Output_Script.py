...
try:
    ...
    print("Warning loading state dict strictly failed:", e)
...
# ---------------- Fit column-wise bias correction ----------------
...
# ----------------- lat/lon and plotting (robust replacement) ---------------
...
# try recursive search (handles Dataset/2024/*.nc)
...
try:
    ...
    # fallback to top-level non-recursive
    try:
...
if lat_name is None or lon_name is None:
    if "t2m" in ds0.variables:
        ...
        if arr.ndim == 3:
            ...
            raise RuntimeError("t2m has unsupported ndim for inferring lat/lon.")
        lats = np.arange(H).astype(float)
        lons = np.arange(W).astype(float)
        origin = "lower"
        print(
            "Warning: lat/lon variables not found; using grid indices for ticks/extent."
        )
    else:
