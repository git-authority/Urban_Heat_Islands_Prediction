...
# ---------------- CONFIG ----------------
# This script is assumed to live inside: Models/SwinLSTM/SwinLSTM_Sliding_v1/
# Dataset is at: ../../Dataset/2024
folder_path = "../../../Dataset/2024"  # 2024 data with t2m
out_dir = "./"  # model folder (SwinLSTM_Sliding_v1)
...
# must match training (SwinLSTM_Sliding_v1)
input_len = 8
target_offset = 4
SAMPLE_STEP = 3
val_split = 0.20  # only used to choose subset for norm stats
seed = 42

hidden_dim = 192
num_layers = 3
kernel_size = 5  # used in refine convs (must match training)
dropout_p = 0.05
...
# ---------------- SwinLSTM model definition (same as training) ----------------
...
class SwinLSTMCell(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        attn_map = self.window_unpartition(attn_out, grid_hw, Hp, Wp)  # B,hidden,Hp,Wp
...
# ---------------- Dataset for evaluation (with timestamps, 2024 only) ----------------
class EvalDataset(torch.utils.data.Dataset):
    """
    Same filling & downsampling logic, plus timestamp for each target (2024 only).
    """
...
# choose subset of 2024 windows to estimate norm stats (similar logic to training)
np.random.seed(seed)
...
# ---------------- Load model ----------------
print("🧠 Loading trained SwinLSTM model weights...")
...
with torch.no_grad():
    for idx in tqdm(range(len(dataset)), ncols=80):
        ...
        pred_abs = pred + clim_t  # 1,1,H,W (absolute units)
...
# ---------------- Plotting helper (stem-style, same as ConvLSTM script) ----------------
...
# Convert any datetime into "2024 with same month/day/time"
