...
hidden_dim = 192
num_layers = 3
kernel_size = 5  # receptive field (used in small convs inside refine)
dropout_p = 0.05

# SwinLSTM-specific
WINDOW_SIZE = 4  # spatial window size for local self-attention
ATTN_HEADS = 4
...
# We implement a Swin-like local-window self-attention that operates on concatenated [x, h] features per time step.
# The attended features are used to compute LSTM gates similarly to ConvLSTMCell.
...
class SwinLSTMCell(nn.Module):
    def __init__(
        ...
        # small projection to combine input and previous hidden to token features
        self.in_proj = nn.Conv2d(input_dim + hidden_dim, hidden_dim, kernel_size=1)
        # the attention module works on flattened tokens per window
        self.attn = LocalWindowAttention(
            dim=hidden_dim, window_size=window_size, num_heads=num_heads
        )
        # gate projections (after attention aggregated back to spatial map)
    ...
    def window_partition(self, x):
        ...
        # reorder to (B, H//ws, W//ws, ws, ws, C)
    ...
    def forward(self, x, hidden):
        ...
        # partition into windows and apply local attention
        # ensure H,W divisible by window_size -> if not, pad (small reflect pad)
        ...
        x_win, grid_hw = self.window_partition(proj_padded)  # B*num_win, N, C
        attn_out = self.attn(x_win)  # same shape
        attn_map = self.window_unpartition(
            attn_out, grid_hw, Hp, Wp
        )  # B, hidden, Hp, Wp
        ...
        # pass through GroupNorm and gates conv
...
# ---------------- Helper losses and metrics (unchanged) ----------------
...
# ---------------- Training loop (unchanged except dataset reference names) ----------------
train_losses = []
val_losses = []
...
# ---------------- Build the requested sample in the DOWNSAMPLED timeline (unchanged section) ----------
