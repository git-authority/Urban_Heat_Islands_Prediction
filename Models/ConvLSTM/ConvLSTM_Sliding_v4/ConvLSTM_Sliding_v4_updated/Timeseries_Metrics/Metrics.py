...
# ---------------- CONFIG (match your ConvLSTM_Sliding_v4_updated config) ----------------
...
# model hyperparams (must match training script)
HIDDEN_DIM = 192
NUM_LAYERS = 3
KERNEL_SIZE = 5
DROPOUT_P = 0.05
...
# ---------------- Dataset (matches SlidingMaskDataset in your script) ----------------
class SlidingMaskDatasetLocal(TorchDataset):
    def __init__(self, folder_path, input_len=8, target_offset=4, sample_step=1):
        ...
        # placeholder timestamps starting 2024-01-01 (UTC) spaced by sample_step hours
...
# ---------------- model classes (must match your training model) ----------------
...
if ckpt is None:
    raise FileNotFoundError(
        "No .pth checkpoint found in MODEL_DIR. Place best_model.pth in MODEL_DIR."
    )
...
save_daily_plot(dates_naive, daily["MAE"].values, "MAE (units)", fig5)
save_daily_plot(dates_naive, daily["RMSE"].values, "RMSE (units)", fig6)
