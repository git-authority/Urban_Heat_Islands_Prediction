# improved_conv_lstm_sampled_updated.py
import os
import numpy as np
...
# try to import SSIM; if unavailable, provide a safe fallback that returns NaN
try:
    from skimage.metrics import structural_similarity as ssim_fn
except Exception:

    def ssim_fn(a, b, **kwargs):
        # skimage not available — return NaN so user sees SSIM couldn't be computed
        return float("nan")


# -------------- CONFIG --------------
folder_path = "../../../Dataset/2024"
out_dir = "ConvLSTM_updated"
...
class NonSlidingMaskDataset(TorchDataset):
    def __init__(self, folder_path, input_len=8, target_offset=4, sample_step=1):
        ...
        # build non-overlapping starts as before (keeps training logic identical)
...
# inference on val set (unchanged)
model.eval()
all_preds = []
all_actuals = []
with torch.no_grad():
...
if preds.size:
    ...
    # compute SSIM averaged over validation samples (fallback to NaN if skimage unavailable)
    combined_all = np.concatenate([actuals.ravel(), preds.ravel()])
    combined_all = combined_all[~np.isnan(combined_all)]
    if combined_all.size == 0:
        val_ssim = float("nan")
    else:
        dr_min = float(np.nanmin(combined_all))
        dr_max = float(np.nanmax(combined_all))
        data_range = dr_max - dr_min if dr_max > dr_min else 1.0
        ssim_vals = []
        for i in range(actuals.shape[0]):
            a = actuals[i, 0]
            p = preds[i, 0]
            fill_val = float(np.nanmean(combined_all))
            a_f = np.nan_to_num(a, nan=fill_val)
            p_f = np.nan_to_num(p, nan=fill_val)
            try:
                s = ssim_fn(a_f, p_f, data_range=data_range)
            except Exception:
                s = float("nan")
            ssim_vals.append(s)
        val_ssim = float(np.nanmean(ssim_vals))

    print("VAL METRICS:", mse, mae, rmse, val_ssim)
else:
...
# inference on the single sample
model.eval()
with torch.no_grad():
    X_sample = torch.from_numpy(inp_filled).unsqueeze(0).float().to(device)
    X_in = (X_sample - norm_mean) / norm_std if USE_NORMALIZATION else X_sample
    out_sample = model(X_in)
    ...
    pred_sample = out_sample.cpu().numpy()[0, 0]
...
# compute SSIM for the sample (fallback to NaN if skimage unavailable)
combined_sample = np.concatenate([actual_sample.ravel(), pred_sample.ravel()])
combined_sample = combined_sample[~np.isnan(combined_sample)]
if combined_sample.size == 0:
    sample_ssim = float("nan")
else:
    dr_min_s = float(np.nanmin(combined_sample))
    dr_max_s = float(np.nanmax(combined_sample))
    data_range_s = dr_max_s - dr_min_s if dr_max_s > dr_min_s else 1.0
    fill_val_s = float(np.nanmean(combined_sample))
    a_f_s = np.nan_to_num(actual_sample, nan=fill_val_s)
    p_f_s = np.nan_to_num(pred_sample, nan=fill_val_s)
    try:
        sample_ssim = float(ssim_fn(a_f_s, p_f_s, data_range=data_range_s))
    except Exception:
        sample_ssim = float("nan")

print(
    f"Sample metrics (downsampled target idx {ds_target_idx}): MSE={mse_sample:.6f}, MAE={mae_sample:.6f}, RMSE={rmse_sample:.6f}, SSIM={sample_ssim}"
)
...
if err_flat.size == 0:
    raise RuntimeError("No valid land pixels for error heatmap.")
err_vmin = float(np.min(err_flat))
err_vmax = float(np.max(err_flat))

# Build ticks: include min and max; include 0.0 only if it's strictly within the (vmin, vmax) range
ticks = [err_vmin]
...
metrics_text = f"MSE: {mse_sample:.6f}   MAE: {mae_sample:.6f}   RMSE: {rmse_sample:.6f}   SSIM: {sample_ssim:.4f}"
...
fig.text(
    ...
    0.02,  # y position (in figure coords) — moved up from 0.02
    metrics_text,
    fontsize=14,
    # fontweight="bold",
    va="center",
