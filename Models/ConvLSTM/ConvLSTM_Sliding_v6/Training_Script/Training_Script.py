...
# ---------------- CONFIG ----------------
# OLD: folder_path = "../../Dataset/2024"  # folder containing .nc files with variable 't2m'
# NEW (use the parent Dataset so all year folders are used)
folder_path = "./Dataset"  # parent folder containing 2020,2021,...,2024 subfolders
out_dir = "ConvLSTM_Sliding_New"
...
GRAD_WEIGHT = (
    0.8  # gradient loss weight (on absolute fields) to reduce large local errors
)
...
class SlidingMaskDataset(TorchDataset):
    def __init__(self, folder_path, input_len=8, target_offset=4, sample_step=1):
        ...
        # NEW: collect .nc files chronologically across year subfolders (year -> month)
        files_paths = []

        # helper month ordering (map lowercase filename -> month index)
        month_map = {
            "january": 1,
            "february": 2,
            "march": 3,
        ...
        # list entries in folder_path
        entries = sorted(os.listdir(folder_path))
        # detect year-subfolders (directories). Try to sort them numerically if possible.
        year_dirs = []
        for e in entries:
            ...
            if os.path.isdir(p):
                year_dirs.append(e)
        if year_dirs:
            # try numeric sort of year names, fallback to lexicographic
            try:
                year_dirs = sorted(year_dirs, key=lambda x: int(x))
            except Exception:
                year_dirs = sorted(year_dirs)
            # for each year, list months in month order
            for y in year_dirs:
                yd = os.path.join(folder_path, y)
                month_files = [f for f in sorted(os.listdir(yd)) if f.endswith(".nc")]

                # sort month_files using month_map; fallback to filename order if unknown
                def month_key(fn):
            ...
            # no year subfolders, fall back to .nc files directly under folder_path
            files_paths = [
        ...
        # finally, check we have files
        if not files_paths:
            raise ValueError(f"No .nc files found in {folder_path}")

        # iterate through files_paths (full paths)
...
# ----------------- REPLACED: CHRONOLOGICAL SPLIT -> YEAR-AWARE SEQUENTIAL SPLIT -----------------
# We assume folder_path contains year subfolders 2020..2024
# Train/Val from 2020-2023 (sequential 80/20), 2024 reserved for test

# Build year directories list (sorted numerically)
year_dirs = sorted(
    [d for d in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, d))],
    key=lambda x: int(x)
)

trainval_years = {"2020", "2021", "2022", "2023"}
test_year = "2024"

# Count frames per year (considering SAMPLE_STEP)
year_frame_counts = {}
for y in year_dirs:
    yd = os.path.join(folder_path, y)
    count = 0
    for fn in sorted(os.listdir(yd)):
        if fn.endswith(".nc"):
            ds = Dataset(os.path.join(yd, fn))
            var = ds.variables.get("t2m")
            if var is None:
                ds.close()
                continue
            arr_shape = var.shape
    ...
    year_frame_counts[y] = count // SAMPLE_STEP

def num_windows(n_frames):
    return max(0, n_frames - input_len - target_offset + 1)

year_window_counts = {y: num_windows(c) for y, c in year_frame_counts.items()}

trainval_count = sum(year_window_counts.get(y, 0) for y in trainval_years)
test_count = year_window_counts.get(test_year, 0)

all_indices = np.arange(n)  # chronological indices 0..n-1

# first chunk corresponds to 2020..2023 windows concatenated, then 2024 windows
trainval_indices = all_indices[:trainval_count]
test_indices = all_indices[trainval_count: trainval_count + test_count]

# sequential 80/20 split inside the trainval block
split_idx = int(np.floor(0.8 * len(trainval_indices)))
train_idx = trainval_indices[:split_idx]
val_idx = trainval_indices[split_idx:]

print(
    f"Train samples (2020–2023, 80%): {len(train_idx)}"
)
print(
    f"Val samples   (2020–2023, 20%): {len(val_idx)}"
)
print(
    f"Test samples  (2024 only):      {len(test_indices)}"
)
...
# create a test loader (2024) if needed later (script currently does val inference / sample plotting)
test_set = Subset(dataset, test_indices)
test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=0)

print(
    f"Samples (downsampled by {SAMPLE_STEP}): total={n}, train={len(train_set)}, val={len(val_set)}, test={len(test_set)}"
)
...
# Ensure the model is saved at the end (safeguard)
...
if preds_blend.size:
    ...
    slope_map, intercept_map = fit_columnwise_bias(
        A, Pblend, dataset.sea_mask, smooth_sigma=2.0
    )
...
def find_first_nc(folder):
    # Use os.walk to recursively search all subfolders (2020, 2021, etc.)
    for root, dirs, files in os.walk(folder):
        for fn in sorted(files):
            if fn.endswith(".nc"):
                return os.path.join(root, fn)
    raise FileNotFoundError(f"No .nc files found in {folder} or subfolders")
