...
out_dir = "ConvLSTM_Sliding_v3_updated"
...
# ---------------- plotting: show only min, 0, max on the error colorbar ----------------
...
# determine vmin_err/vmax_err for the error panel (fallback if no valid values)
if err_vals.size == 0:
    vmin_err, vmax_err = -0.5, 0.5
else:
    vmin_err = float(np.nanmin(err_vals))
    vmax_err = float(np.nanmax(err_vals))
    # ensure a tiny padding if min==max
    if np.isclose(vmin_err, vmax_err):
        pad = max(1e-3, abs(vmin_err) * 0.01)
        vmin_err -= pad
        vmax_err += pad

# Build ticks: min, 0, max (sorted ascending)
err_ticks = np.array([vmin_err, 0.0, vmax_err], dtype=float)
err_ticks = np.unique(np.round(err_ticks, 6))  # remove duplicates/very-close values
err_ticks = np.sort(err_ticks)

# create figure (same layout as before)
...
# error image uses vmin_err/vmax_err so color mapping covers actual data range
im2 = axes[2].imshow(
    error_masked,
    origin=origin,
    extent=extent,
...
for ax, im in zip(axes, [im0, im1, im2]):
    ...
    if im is im2:
        ...
        # set ticks to only [min, 0, max] (we computed err_ticks above)
        cbar.set_ticks(err_ticks)
        # format tick labels with 3 decimals (adjust if you want fewer/more)
