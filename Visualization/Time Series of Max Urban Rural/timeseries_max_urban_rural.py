#!/usr/bin/env python3
"""
Daily MAX (per-timepoint mask max → daily mean/std) plots per year for Rural & Urban.
...
# -------- compute per-timestep MAX for a mask ----------
def compute_max_timeseries_for_mask(files, mask):
    times = []
    vals = []
        ...
        for ti in range(use_nt):
            ...
            if arr.shape != mask.shape:
                try:
                    ...
                    arr = arr.flatten()
                    m = mask.flatten()
                    vals.append(np.nanmax(arr[m]))
                    times.append(dt)
                    continue

            vals.append(np.nanmax(arr[mask]))
            times.append(dt)

        ds.close()

    return pd.DataFrame({"max": vals}, index=pd.to_datetime(times)).sort_index()


# -------- main ----------
def main():
    out_dir = "Daily_Per_Year_Max"
    ...
    for year in YEARS:
        ...
        rural_ts = compute_max_timeseries_for_mask(files, rural_mask)
        urban_ts = compute_max_timeseries_for_mask(files, urban_mask)

        # Daily mean of per-timepoint MAX, and daily STD
        rural_daily_max = rural_ts["max"].resample("D").mean()
        rural_daily_std = rural_ts["max"].resample("D").std(ddof=0)

        urban_daily_max = urban_ts["max"].resample("D").mean()
        urban_daily_std = urban_ts["max"].resample("D").std(ddof=0)

        day_idx = rural_daily_max.index.union(urban_daily_max.index).sort_values()
        ...
        ax.fill_between(
            day_idx,
            rural_daily_max.reindex(day_idx) - rural_daily_std.reindex(day_idx),
            rural_daily_max.reindex(day_idx) + rural_daily_std.reindex(day_idx),
            alpha=0.22,
            color="tab:blue",
            label="Rural daily max ±STD",
        )
        ax.plot(
            day_idx,
            rural_daily_max.reindex(day_idx),
            lw=1.8,
            color="tab:blue",
            label="Rural daily max",
        )

        # Urban shading
        ax.fill_between(
            day_idx,
            urban_daily_max.reindex(day_idx) - urban_daily_std.reindex(day_idx),
            urban_daily_max.reindex(day_idx) + urban_daily_std.reindex(day_idx),
            alpha=0.2,
            color="tab:red",
            label="Urban daily max ±STD",
        )
        ax.plot(
            day_idx,
            urban_daily_max.reindex(day_idx),
            lw=1.8,
            color="tab:red",
            label="Urban daily max",
        )

        ax.text(
            0.02,
            0.88,
            f"Mean of Rural of Daily Max STD: {rms:.2f} K\nMean of Urban Daily Max STD: {ums:.2f} K",
        ...
        ax.set_title(
            f"Mumbai ({year}) - Daily Average of Hourly Maxima for t2m (Jan-Dec)",
            fontsize=14,
            weight="bold",
        )
        ax.set_ylabel("Daily Max Temperature (K)")
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"{year}_daily_max.png"), dpi=180)
        plt.close(fig)

        # Save CSVs
        pd.DataFrame(
            {"daily_max": rural_daily_max, "daily_std": rural_daily_std}
        ).to_csv(os.path.join(out_dir, f"{year}_rural_daily_max.csv"))

        pd.DataFrame(
            {"daily_max": urban_daily_max, "daily_std": urban_daily_std}
        ).to_csv(os.path.join(out_dir, f"{year}_urban_daily_max.csv"))

        print("Saved:", os.path.join(out_dir, f"{year}_daily_max.png"))
