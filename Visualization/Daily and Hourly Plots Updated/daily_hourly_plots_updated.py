...
def make_and_save_separate_figures(data_root, years):
    ...
    output_dir = os.path.join(os.getcwd(), "Daily and Hourly Plots Updated")
    ...
    for year in years:
        ...
        fig_h.tight_layout()
        fig_h.savefig(os.path.join(output_dir, out_h), dpi=180)
        plt.close(fig_h)
        ...
        ax_d.set_title(
            f"{year} — Daily average t2m (Jan–Dec)", fontsize=12, weight="bold"
        )
        ...
        fig_d.tight_layout()
        fig_d.savefig(os.path.join(output_dir, out_d), dpi=180)
        plt.close(fig_d)

        print(
            f"Saved: {os.path.join(output_dir, out_h)}  and  {os.path.join(output_dir, out_d)}"
        )
